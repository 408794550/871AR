...
namespace Lean.Touch
{
	...
	public partial class LeanTouch : MonoBehaviour
	{
		...
		//=====
		public LeanPressSelect leanPressSelect;
...
//				if (leanPressSelect.CurrentSelectable == null && Input.touchCount == 1 ) {
//					//AddFinger(2,Input.GetTouch(0).position);
//					//GameObject.Find("871AR_KaGuo_Nai_Effect").transform.position;
//					var center = new Vector2 (Screen.width * 0.5f, Screen.height * 0.5f);
//
//					var center2 = center - (Input.GetTouch (0).position - center);
//					AddFinger (999, center2);
//
//					Debug.Log ("----" + center2 + "+++++" + Input.GetTouch (0).position);
//				} 
			}
				...
				if (screen.Contains(mousePosition) == true)
				{
					...
					if (SimulateMultiFingers == true)
					{
						if (Input.GetKey(PinchTwistKey) == true)
						{
							...
							AddFinger(999, center - (mousePosition - center));
						}
						// Simulate multi drag?
						else if (Input.GetKey(MultiDragKey) == true)
						{
							AddFinger(999, mousePosition);
						}
					}
				}
			}
